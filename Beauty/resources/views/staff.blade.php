...
<div class="relative bg-cover bg-center h-64" style="background-image: url('{{ asset('css/images/staff.png') }}');">
...
<div class="my-10 container mx-auto">
    <h2 class="text-3xl font-semibold mb-4">Add New Staff</h2>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
        @foreach ($staffs as $staff)
            <div class="max-w-sm rounded overflow-hidden shadow-lg bg-white">
                <img class="w-full h-64 object-cover" src="{{ asset('images/' . $staff->image_path) }}" alt="{{ $staff->artist_name }}">
                <div class="px-6 py-4">
                ...
                @if(Auth::check() && Auth::user()->isAdmin())
                    <div class="px-6 pt-4 pb-2">
                        <a href="{{ route('staff.edit', $staff->artist_name) }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Edit</a>
                    </div>
                @endif
            </div>
        @endforeach
