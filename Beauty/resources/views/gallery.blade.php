@extends('layouts.app')
@section('content')
<div class="container mx-auto px-5 lg:px-32 lg:pt-24">
    <div class="flex flex-wrap -mx-1 md:-mx-2">
        <div class="flex w-1/2 flex-wrap">
            <div class="w-1/2 p-1 md:p-2">
                <img alt="gallery" class="block h-full w-full rounded-lg object-cover object-center" src="https://i.pinimg.com/236x/d9/a9/3f/d9a93f4d7bb5a0e89edb057a6c8079ee.jpg" />
            </div>
            <div class="w-1/2 p-1 md:p-2">
                <img alt="gallery" class="block h-full w-full rounded-lg object-cover object-center" src="https://i.pinimg.com/236x/3b/94/eb/3b94eb45500ddef827b40f2ee17bf2b2.jpg" />
            </div>
            <div class="w-full p-1 md:p-2">
                <img alt="gallery" class="block h-full w-full rounded-lg object-cover object-center" src="https://www.hindustantimes.com/ht-img/img/2023/02/13/original/3f40a9a29dbf219e279b223661590174_1676286305930.jpg" />
            </div>
        </div>
        <div class="flex w-1/2 flex-wrap">
            <div class="w-full p-1 md:p-2">
                <img alt="gallery" class="block h-full w-full rounded-lg object-cover object-center" src="https://www.uptowngirl.com/wp-content/uploads/2022/09/cat-nail-art-designs-04.jpg" />
            </div>
            <div class="w-1/2 p-1 md:p-2">
                <img alt="gallery" class="block h-full w-full rounded-lg object-cover object-center" src="https://i.pinimg.com/736x/29/44/12/2944122bb1fa129fd5e7ce4cda023ee2.jpg" />
            </div>
            <div class="w-1/2 p-1 md:p-2">
                <img alt="gallery" class="block h-full w-full rounded-lg object-cover object-center" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRyQukTXE72gJzzmcMCCD5taZFTdg4z3Hh8qg&usqp=CAU" />
            </div>
        </div>

        <div class="flex w-1/2 flex-wrap">
            <div class="w-1/2 p-1 md:p-2">
                <img alt="gallery" class="block h-full w-full rounded-lg object-cover object-center" src="https://i.pinimg.com/originals/e1/43/60/e143601fea1b0c216e50dd12abea2464.jpg" />
            </div>
            <div class="w-1/2 p-1 md:p-2">
                <img alt="gallery" class="block h-full w-full rounded-lg object-cover object-center" src="https://i.pinimg.com/736x/fa/80/eb/fa80ebf6d72a60f5ae4ef326511f8b17.jpg" />
            </div>
            <div class="w-full p-1 md:p-2">
                <img alt="gallery" class="block h-full w-full rounded-lg object-cover object-center" src="https://themoodguide.com/wp-content/uploads/2023/01/long-square-nails-with-brown-design_amanda.sudolll_the-mood-guide.jpg" />
            </div>
        </div>
        <div class="flex w-1/2 flex-wrap">
            <div class="w-full p-1 md:p-2">
                <img alt="gallery" class="block h-full w-full rounded-lg object-cover object-center" src="https://m.media-amazon.com/images/I/71VQQ+floXL._AC_UF1000,1000_QL80_.jpg" />
            </div>
            <div class="w-1/2 p-1 md:p-2">
                <img alt="gallery" class="block h-full w-full rounded-lg object-cover object-center" src="https://i.pinimg.com/1200x/98/70/a2/9870a25aa3c18659110c193f87f0e549.jpg" />
            </div>
            <div class="w-1/2 p-1 md:p-2">
                <img alt="gallery" class="block h-full w-full rounded-lg object-cover object-center" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQaoVHh24JqdaO6_Rhc8tTB1f4j15sPmt800w&usqp=CAU" />
            </div>
        </div>

        <div class="flex w-1/2 flex-wrap">
            <div class="w-1/2 p-1 md:p-2">
                <img alt="gallery" class="block h-full w-full rounded-lg object-cover object-center" src="https://abeautifulmess.com/wp-content/uploads/2024/01/spring-nail-ideas-pink-french-chrome-barbie-1-1024x1024.jpg" />
            </div>
            <div class="w-1/2 p-1 md:p-2">
                <img alt="gallery" class="block h-full w-full rounded-lg object-cover object-center" src="https://i.etsystatic.com/34389166/r/il/f5974f/3711798859/il_570xN.3711798859_jbbw.jpg" />
            </div>
            <div class="w-full p-1 md:p-2">
                <img alt="gallery" class="block h-full w-full rounded-lg object-cover object-center" src="https://i.pinimg.com/736x/43/87/82/4387826a7db81459057e314c02fddf97.jpg" />
            </div>
        </div>
        <div class="flex w-1/2 flex-wrap">
            <div class="w-full p-1 md:p-2">
                <img alt="gallery" class="block h-full w-full rounded-lg object-cover object-center" src="https://www.salonsdirect.com/blog/wp-content/uploads/2022/03/envybeautyexeter.jpg" />
            </div>
        </div>
    </div>
</div>

@endsection
